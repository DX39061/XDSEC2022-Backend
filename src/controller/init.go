...
func Initialize() error {
	...
	engine = gin.New()
	engine.Use(GinZap(zap.L(), "", true))
	engine.Use(RecoveryWithZap(zap.L(), true))
	SetupRouting(engine)
	return nil
}

func Run() error {
	logger.Info("Service running at [ " + config.ServerConfig.Address + ":" + strconv.Itoa(config.ServerConfig.Port) + " ]")
	err := engine.Run(config.ServerConfig.Address + ":" + strconv.Itoa(config.ServerConfig.Port))
	if err != nil {
		return err
	}
	return nil
